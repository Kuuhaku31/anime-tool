...
#[derive(Debug, Parser)]
#[command(about = "根据 Markdown meta 的 match/get/to 从 JSON 更新 fenced 区域")]
struct Args {
    /// 需要原地编辑的 Markdown 文件
    input: PathBuf,
}
...
fn process_section(
    ...
    let Some(get_value) = meta.get("get") else {
        return Ok(false);
    };
    let Some(to_value) = meta.get("to") else {
        return Ok(false);
    };
    let fields: Vec<String> = get_value.split_whitespace().map(str::to_string).collect();
    if fields.is_empty() {
        return Ok(false);
    }
    ...
    let source = source
        .as_object()
        .ok_or_else(|| anyhow!("match 最终结果必须是 JSON 对象"))?;
    let target = to_value.trim();

    if let Some((block_start, block_end)) =
        markdown::find_named_code_block(lines, start, end, target)
    {
        let mut content = lines[block_start + 1..block_end].to_vec();
        markdown::update_named_block(&mut content, &fields, source);
    ...
    for key in fields {
        if let Some(value) = source.get(&key) {
            insertion.push(format!(
                "{}: {}",
                key,
                markdown::normalize_output_value(value)
            ));
        }
    }
